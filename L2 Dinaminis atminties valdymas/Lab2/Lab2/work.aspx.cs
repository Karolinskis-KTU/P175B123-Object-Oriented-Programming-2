        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            if (FileUpload1.HasFile && FileUpload2.HasFile)    
            {
                ...
                if (extension1.Equals(".txt") && extension2.Equals(".txt"))
                {
                    ...
                    ProjectLinkedList ProjectData = InOut.ReadProjectFile(path1);
                    StudentLinkedList StudentsData = InOut.ReadStudentFile(path2);
                    ...
                    ProjectLinkedList ActiveProffessors = TaskUtils.GetAllActiveProffesors(ProjectData, InactiveProjects);

                    var ActiveProffesorsNode = ActiveProffessors.Head;
                    while (ActiveProffesorsNode != null)
                    {
                        Project prof = ActiveProffesorsNode.data as Project;
                        Table1.Rows.Add(DataOutput.MakeRow(prof.ProfSurname, prof.ProfName));

                        ActiveProffesorsNode = ActiveProffesorsNode.next; // Itterate while
                    }
